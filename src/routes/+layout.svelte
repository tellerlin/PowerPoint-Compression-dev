<script>
    ...
    const nonce = crypto.randomUUID(); // 生成随机 nonce

    onMount(() => {
        // 加载关键资源
        loadPreloadResources(preloadResources);

        // 初始化 Google Analytics 和 WeChat Meta Tags
        initializeGoogleAnalytics(nonce);
        initializeWeChatMetaTags(siteMetadata, nonce);
    });
</script>

<svelte:head>

    <meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.googletagmanager.com https://res.wx.qq.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; connect-src 'self' https://www.google-analytics.com">
